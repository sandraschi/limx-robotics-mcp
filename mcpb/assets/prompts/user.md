...
2. **LimX MuJoCo** installed on your system
3. Repository cloned: `git clone https://github.com/sandraschi/limx-robotics-mcp.git`
4. Dependencies installed: `cd limx-robotics-mcp && uv sync`
...
then opens the dashboard in your browser at http://localhost:11045.
...
```bash
uv run python -c "from limx_robotics_mcp.server import sim_status; print(sim_status())"
```
...
### Ports
- Backend API: 11044 (FastAPI REST + MCP HTTP endpoints)
- Frontend Dashboard: 11045 (Vite React SPA)
...
### 1. "LimX MuJoCo" not available from sim_status()
Ensure LimX MuJoCo is installed and accessible. Check that the binary or Python module
...
```
If ports 11044 or 11045 are in use, kill the occupying processes and restart.
...
- Use stop_sim() to load
- Use run_deployed_policy() to verify it is present
...
| sim_jobs() | Sim | ... | {success, ...} |
| list_robot_variants() | Sim | ... | {success, ...} |
| get_robot_description() | Sim | ... | {success, ...} |
| export_model_for_fleet() | Sim | ... | {success, ...} |
| list_policies() | Sim | ... | {success, ...} |
| run_deployed_policy() | Sim | ... | {success, ...} |
...
1. **First launch latency**: The first LimX MuJoCo launch may be significantly slower due to
