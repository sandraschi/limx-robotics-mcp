...
export default function Policies() {
	...
	useEffect(() => {
		fetch(`${API_BASE}/api/policies?platform=${platform}`)
			.then((r) => r.json())
			.then((d) => {
	...
	const runPolicy = () => {
		setRunning(true);
		setResult(null);
		fetch(API_BASE + "/api/policies/run", {
			method: "POST",
			...
			.catch((e) => setResult({ exit_code: -1, log: String(e) }))
				...
				{result && (
					<div className="mt-4 space-y-2">
						<div className="flex items-center gap-2 text-sm">
							...
							<span
								className={
									result.exit_code === 0 ? "text-green-600" : "text-red-600"
								}
							>
								{result.exit_code}
							</span>
