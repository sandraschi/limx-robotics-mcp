...
export default function Simulations() {
	...
	useEffect(() => {
		fetch(API_BASE + "/api/models/variants")
			.then((r) => r.json())
			.then((d) => {
	...
	const toggleExpand = (jobId: string) => {
		...
		setExpanded(jobId);
		setLogTail("Loading…");
		fetch(`${API_BASE}/api/sim/jobs?job_id=${jobId}&log_tail_lines=100`)
			.then((r) => r.json())
	...
	const analyzeJob = async (jobId: string) => {
		...
		setAiResult("");
		try {
			const r = await fetch(API_BASE + "/api/ai/analyze-state", {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ job_id: jobId }),
			});
