...
const TOOLS = [
	{
		name: "sim_status",
		desc: "Composite health check (env, submodules, GPU)",
		group: "Core Sim",
	},
	{
		name: "start_sim",
		desc: "Launch MuJoCo sim (TRON 1 or Oli MuJoCo)",
		group: "Core Sim",
	},
	{
		name: "stop_sim",
		desc: "Terminate a running sim by job_id",
		group: "Core Sim",
	},
	{
		name: "sim_jobs",
		desc: "List/query active and completed sim jobs",
		group: "Core Sim",
	},
	{
		name: "list_robot_variants",
		desc: "Discover available robot model variants",
		group: "Core Sim",
	},
	{
		name: "get_robot_description",
		desc: "Get URDF/USD/XML model file content",
		group: "Core Sim",
	},
	{
		name: "list_policies",
		desc: "List available RL/VLA policies",
		group: "Core Sim",
	},
	{
		name: "run_deployed_policy",
		desc: "Deploy a trained RL policy to a sim",
		group: "Core Sim",
	},
	{
		name: "export_model_for_fleet",
		desc: "Export robot as GLB for Godot/Unity",
		group: "Core Sim",
	},
	{
		name: "agentic_sim_workflow",
		desc: "Autonomous multi-step sim via LLM",
		group: "AI Workflow",
	},
	{
		name: "natural_language_control",
		desc: "Make it crouch → actuator values",
		group: "AI Workflow",
	},
	{
		name: "analyze_sim_state",
		desc: "LLM describes robot behaviour from state",
		group: "AI Workflow",
	},
	{
		name: "analyze_sim_logs",
		desc: "LLM diagnoses issues from sim logs",
		group: "AI Workflow",
	},
	{
		name: "discover_model",
		desc: "LLM generates URLs, downloads MJCF models",
		group: "AI Workflow",
	},
];
...
const TROUBLES = [
	{
		...
		fix: 'Run .\\setup-sim-env.ps1 from the repo root. Verify: .venv-sim38\\Scripts\\python.exe -c "import mujoco"',
	},
	{
		symptom: "OSError: python38.dll not found",
		cause: "Sim running under MCP's Python (3.11+) instead of sim interpreter",
		fix: "Check LIMX_SIM_PYTHON env var. Default: .venv-sim38\\Scripts\\python.exe. Must point to Python 3.8.",
	},
	{
		symptom: "FileNotFoundError: robot.xml",
		cause: "Submodules not initialized in upstream sim repo",
		fix: "git submodule update --init --recursive in tron1-mujoco-sim or humanoid-mujoco-sim.",
	},
	{
		symptom: "Failed to launch simulator",
		cause: "simulator.py not found — repo not cloned",
		fix: "Clone upstream repos with --recurse-submodules to the external directory.",
	},
	{
		symptom: "MuJoCo viewer opens and immediately closes",
		cause: "Missing model file or invalid ROBOT_TYPE",
		fix: "Check the log via sim_jobs(job_id, log_tail_lines=50). Verify ROBOT_TYPE with list_robot_variants().",
	},
	{
		symptom: "Process hangs on wait()",
		cause: "MuJoCo viewer window still open",
		fix: "Close the viewer window or use stop_sim() which sends SIGTERM then SIGKILL after 5s.",
	},
	{
		symptom: "GLB export fails: trimesh not installed",
		cause: "Missing trimesh package in server venv",
		fix: "Run: uv pip install trimesh",
	},
	{
		symptom: "Web dashboard shows Not Ready",
		cause: "One or more external repos or submodules missing",
		fix: "Check Dashboard cards for red indicators. Ensure all required repos exist.",
	},
	{
		symptom: "Policy deploy fails with no controllers",
		cause: "RL deploy repo not cloned or empty controllers/",
		fix: "Clone tron1-rl-deploy-python or humanoid-rl-deploy-python to external/. Controllers dir populated by training.",
	},
	{
		symptom: "list_robot_variants returns empty",
		cause: "Submodule not populated",
		...
		<div className="space-y-4">
			<Card title="What It Is">
				<p className="text-sm text-slate-600 mb-2">
					<strong>limx-robotics-mcp</strong> is a FastMCP 3.2 wrapper for LimX
					Dynamics' open-source robotics stack — TRON 1 biped and Oli humanoid.
					Run MuJoCo simulations, deploy VLA policies, and visualize robots in
					Godot/Unity — no hardware required.
				</p>
				<p className="text-sm text-slate-600">
					The MCP server orchestrates LimX's upstream repos. Simulations launch
					as managed background processes under a dedicated{" "}
					<strong>Python 3.8</strong> interpreter (the LimX SDK links
					<code className="text-xs bg-slate-100 px-1 rounded">
						{" "}
						python38.dll
					</code>
					). The server itself runs on Python 3.11+.
				</p>
			</Card>
...
Web Dashboard (Vite + React, :11045)
    └── backend (FastAPI, :11044)`}
				...
				<p className="text-sm text-slate-600">
					<strong>Two Python interpreters:</strong> Server on 3.11–3.12
					(FastMCP, FastAPI), sim on Python 3.8 (MuJoCo + LimX SDK).
				</p>
			</Card>
		...
		<div className="space-y-4">
			<Card title="Prerequisites">
				<ul className="list-disc list-inside text-sm text-slate-600 space-y-1">
					<li>
						<strong>Python 3.11–3.12</strong> — for the MCP server (FastMCP
						requirement)
					</li>
					<li>
						<strong>Python 3.8</strong> — for the sim interpreter (SDK links
						python38.dll)
					</li>
					<li>
						<strong>MuJoCo 3.1.6</strong> — installed in the sim venv
					</li>
					<li>
						<strong>Git</strong> — with submodule support
					</li>
					<li>
					...
					<li>
						<strong>Upstream repos</strong> — cloned to{" "}
						<code className="text-xs bg-slate-100 px-1 rounded">
					...
					<table className="w-full text-sm">
						...
						<tbody>
							<tr className="border-b border-slate-100">
								...
								<td className="py-2 text-xs text-slate-600">
									Directory with all upstream LimX repos
								</td>
							</tr>
							<tr className="border-b border-slate-100">
								<td className="py-2 pr-4 text-xs font-mono">LIMX_SIM_PYTHON</td>
								<td className="py-2 pr-4 text-xs text-slate-500">
									.venv-sim38/Scripts/python.exe
								</td>
								<td className="py-2 text-xs text-slate-600">
									Python 3.8 interpreter for sim processes
								</td>
							</tr>
							<tr className="border-b border-slate-100">
	...
	return (
		<Card title="Common Issues">
			...
			<div className="mt-4 p-3 bg-slate-50 rounded text-xs text-slate-600">
				<p className="mb-1">
					<strong>Log files:</strong> Per-simulation log in{" "}
					<code className="text-xs bg-slate-100 px-1 rounded">
						logs/&lt;job_id&gt;.log
					</code>
				</p>
				<p className="mb-1">
					<strong>Reset:</strong> Re-run{" "}
					<code className="text-xs bg-slate-100 px-1 rounded">
						.\setup-sim-env.ps1
					</code>{" "}
					and{" "}
