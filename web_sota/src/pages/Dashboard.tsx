...
export default function Dashboard() {
	...
	const runAiWorkflow = async () => {
		...
		setAiResult("");
		try {
			const r = await fetch(API_BASE + "/api/ai/workflow", {
				method: "POST",
		...
		<div className="p-6 space-y-6">
				...
				<Card title="Sim Environment">
					<div className="text-sm space-y-2">
						<div className="flex items-center gap-2">
							<span
								className={`w-2 h-2 rounded-full ${
									status.sim_environment.deps_ok ? "bg-green-500" : "bg-red-500"
								}`}
							/>
			...
			<Card title="Quick Actions">
				<div className="flex gap-3 flex-wrap">
					<button
						onClick={() =>
							fetch(API_BASE + "/api/sim/start", { method: "POST" }).catch(() => {})
						}
					...
					<button
						onClick={() =>
							fetch(API_BASE + "/api/sim/stop", { method: "POST" }).catch(() => {})
						}
					...
					<button
						onClick={() =>
							fetch(API_BASE + "/api/models/export", { method: "POST" }).catch(() => {})
						}
