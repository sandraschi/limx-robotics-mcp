...
export default function Models() {
	...
	useEffect(() => {
		fetch(`${API_BASE}/api/models/variants?platform=${platform}`)
			.then((r) => r.json())
			.then((d) => {
	...
	useEffect(() => {
		if (!selected) return;
		fetch(`${API_BASE}/api/models/descriptions?platform=${platform}&variant=${selected}`)
			.then((r) => r.json())
			.then((d) => {
	...
	useEffect(() => {
		fetch(API_BASE + "/api/models/depot")
			.then((r) => r.json())
			.then((d) => {
				const list = Array.isArray(d.files) ? d.files : [];
				setDepot(list);
	...
	const doExport = () => {
		setExportMsg("");
		fetch(API_BASE + "/api/models/export", {
			method: "POST",
